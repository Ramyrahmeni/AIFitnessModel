...
class ProcessFrame:
    def __init__(self, thresholds, flip_frame = False):
        ...
        self.FEEDBACK_ID_MAP = {
                                0: ('CORRECT YOUR ELBOWS', 215, (0, 153, 255)),
                                1: ('TOO DEEP', 215, (0, 153, 255)),
                               }
    ...
    def _update_state_sequence(self, state):
        ...
        if state == 's2':
            ...
            if (state not in self.state_tracker['state_seq']) and 's2' in self.state_tracker['state_seq']: 
                self.state_tracker['state_seq'].append(state)
    def _show_feedback(self, frame, c_frame, dict_maps, lower_body):
        ...
        if keypoints.pose_landmarks:
            ...
            if offset_angle > self.thresholds['OFFSET_THRESH']:
                ...
                if self.state_tracker['INACTIVE_TIME_FRONT'] >= self.thresholds['INACTIVE_THRESH']:
                    self.state_tracker['DIP_COUNT'] = 0
                    self.state_tracker['IMPROPER_DIP'] = 0
                ...
                draw_text(
                    frame, 
                    "CORRECT: " + str(self.state_tracker['DIP_COUNT']), 
                ...
                draw_text(
                    frame, 
                    "INCORRECT: " + str(self.state_tracker['IMPROPER_DIP']), 
            ...
            # Camera is aligned properly.
                ...
                elbow_angle = find_angle(wrist_coord,shldr_coord, elbow_coord)
                ...
                if current_state == 's1':
                    ...
                    if len(self.state_tracker['state_seq']) == 3 and not self.state_tracker['ELBOWS_OUT']:
                        self.state_tracker['DIP_COUNT']+=1
                        play_sound = str(self.state_tracker['DIP_COUNT'])
                        
                    elif 's2' in self.state_tracker['state_seq'] and len(self.state_tracker['state_seq'])==1:
                        self.state_tracker['IMPROPER_DIP']+=1
                        play_sound = 'incorrect'

                    elif self.state_tracker['ELBOWS_OUT']:
                        self.state_tracker['IMPROPER_DIP']+=1
                        play_sound = 'incorrect'
                        
                    
                    self.state_tracker['state_seq'] = []
                    self.state_tracker['ELBOWS_OUT'] = False
                ...
                # -------------------------------------- PERFORM FEEDBACK ACTIONS --------------------------------------
                    ...
                    if elbow_angle < self.thresholds['ELBOW_THRESH']:
                        self.state_tracker['DISPLAY_TEXT'][0] = True
                        

                    elif elbow_angle < self.thresholds['DEEP_THRESH'][1] and \
                         self.state_tracker['state_seq'].count('s2')==1:
                            self.state_tracker['DISPLAY_TEXT'][1] = True
                        
                                        
                    
                    if elbow_angle > self.thresholds['SHOULDER_ELBOW_WRIST']['NORMAL'][1] and \
                       self.state_tracker['state_seq'].count('s2')==1:
                        self.state_tracker['LOWER_BODY'] = True
                    ...
                    if (elbow_angle < self.thresholds['DEEP_THRESH'][1]):
                        self.state_tracker['DISPLAY_TEXT'][0] = True
                        self.state_tracker['ELBOWS_OUT'] = True
                ...
                if 's3' in self.state_tracker['state_seq'] or current_state == 's1':
                    self.state_tracker['LOWER_BODY'] = False

                self.state_tracker['COUNT_FRAMES'][self.state_tracker['DISPLAY_TEXT']]+=1

                frame = self._show_feedback(frame, self.state_tracker['COUNT_FRAMES'], self.FEEDBACK_ID_MAP, self.state_tracker['LOWER_BODY'])
                ...
                cv2.putText(frame, str(int(elbow_angle)), (elbow_text_coord_x, elbow_coord[1]), self.font, 0.6, self.COLORS['light_green'], 2, lineType=self.linetype)
