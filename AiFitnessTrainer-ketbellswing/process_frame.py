...
class ProcessFrame:
    def __init__(self, thresholds, flip_frame = False):
        ...
        self.FEEDBACK_ID_MAP = {
                                2: ('KNEE FALLING OVER TOE', 215, (0, 153, 255)),
                                1: ('KEEP ARMS STRAIGHT', 215, (0, 153, 255)),
                                0: ('RAISE ARMS', 170, (255, 80, 80)),
                               }
    def _get_state(self, shoulder_angle):
        
        knee = None        

        if self.thresholds['HIP_KNEE_VERT']['NORMAL'][0] <= shoulder_angle <= self.thresholds['HIP_KNEE_VERT']['NORMAL'][1]:
            knee = 1
        elif self.thresholds['HIP_KNEE_VERT']['TRANS'][0] <= shoulder_angle <= self.thresholds['HIP_KNEE_VERT']['TRANS'][1]:
            knee = 2
        elif self.thresholds['HIP_KNEE_VERT']['PASS'][0] <= shoulder_angle <= self.thresholds['HIP_KNEE_VERT']['PASS'][1]:
            knee = 3
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            if offset_angle > self.thresholds['OFFSET_THRESH']:
                ...
                draw_text(
                    frame, 
                    "CORRECT: " + str(self.state_tracker['SWING_COUNT']), 
                ...
                draw_text(
                    frame, 
                    "INCORRECT: " + str(self.state_tracker['IMPROPER_SWING']), 
            ...
            # Camera is aligned properly.
                ...
                shoulder_vertical_angle = find_angle(np.array([shldr_coord[0], 0]),shldr_coord,  elbow_coord)
                ...
                current_state = self._get_state(int(shoulder_vertical_angle))
                ...
                if current_state == 's1':
                    ...
                    if len(self.state_tracker['state_seq']) == 3 and not self.state_tracker['INCORRECT_ELBOW']and not self.state_tracker['KNEE_INLINE']:
                        self.state_tracker['SWING_COUNT']+=1
                        play_sound = str(self.state_tracker['SWING_COUNT'])
                        
                    elif 's2' in self.state_tracker['state_seq'] and len(self.state_tracker['state_seq'])==1:
                        self.state_tracker['IMPROPER_SWING']+=1
                        play_sound = 'incorrect'

                    elif self.state_tracker['KNEE_INLINE'] or self.state_tracker['INCORRECT_ELBOW'] :
                        self.state_tracker['IMPROPER_SWING']+=1
                        play_sound = 'incorrect'
                        
                    
                    self.state_tracker['state_seq'] = []
                    self.state_tracker['INCORRECT_ELBOW'] = False
                    self.state_tracker['KNEE_INLINE']=False
                ...
                # -------------------------------------- PERFORM FEEDBACK ACTIONS --------------------------------------
                    ...
                    if self.thresholds['ELBOW_SHOULDER_VERT']['TRANS'][0] < shoulder_vertical_angle < ['ELBOW_SHOULDER_VERT']['TRANS'][1] and \
                       self.state_tracker['state_seq'].count('s2')==0:
                        self.state_tracker['RAISE_ARMS'] = True
                        self.state_tracker['DISPLAY_TEXT'][0] = True



                    elif elbow_angle < self.thresholds['ELBOW_THRESH']:
                        self.state_tracker['DISPLAY_TEXT'][1] = True
                        self.state_tracker['INCORRECT_ELBOW'] = True

                    
                    if (ankle_vertical_angle > self.thresholds['ANKLE_THRESH']):
                        self.state_tracker['DISPLAY_TEXT'][2] = True
                        self.state_tracker['KNEE_INLINE'] = True
                ...
                if self.state_tracker['curr_state'] == self.state_tracker['prev_state']:
                    ...
                    if self.state_tracker['INACTIVE_TIME'] >= self.thresholds['INACTIVE_THRESH']:
                        self.state_tracker['SWING_COUNT'] = 0
                        self.state_tracker['IMPROPER_SWING'] = 0
                        display_inactivity = True
                ...
                if 's3' in self.state_tracker['state_seq'] or current_state == 's1':
                    self.state_tracker['RAISE_ARMS'] = False
                ...
                cv2.putText(frame, str(int(shoulder_vertical_angle)), (hip_text_coord_x, hip_coord[1]), self.font, 0.6, self.COLORS['light_green'], 2, lineType=self.linetype)
                cv2.putText(frame, str(int(elbow_angle)), (knee_text_coord_x, knee_coord[1]+10), self.font, 0.6, self.COLORS['light_green'], 2, lineType=self.linetype)
