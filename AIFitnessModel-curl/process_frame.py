...
class ProcessFrame:
    def __init__(self, thresholds, flip_frame = False):
        ...
        self.state_tracker = {
            ...
            'DISPLAY_TEXT' : np.full((1,), False),
            'COUNT_FRAMES' : np.zeros((1,), dtype=np.int64),
    ...
    def _show_feedback(self, frame, c_frame, dict_maps, hold_weight, curl_slowly):
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            # Camera is aligned properly.
                ...
                elbow_shoulder_hip = find_angle(elbow_coord, np.array([hip_coord[0], 0]),hip_coord)
                cv2.ellipse(frame, hip_coord, (30, 30), 
                            angle = 0, startAngle = -90, endAngle = -90+multiplier*elbow_shoulder_hip, 
                            color = self.COLORS['white'], thickness = 3, lineType = self.linetype)
                ...
                # -------------------------------------- PERFORM FEEDBACK ACTIONS --------------------------------------
                     ...
                     #keep holding the weight and curl slowly : s1-->s2 WRIST_ELBOW_VERT in [160,180]                   
                    if  elbow_vertical_angle > self.thresholds['WRIST_ELBOW_VERT']['NORMAL'][0] :
                    #    self.state_tracker['state_seq'].count('s1')==0
                        self.state_tracker['HOLD_WEIGHT_AND_CURL'] = True  

                   #CURL your arm :s2-->s3    WRIST_ELBOW_VERT in [80,120]
                    elif  elbow_vertical_angle > self.thresholds['WRIST_ELBOW_VERT']['TRANS'][0] and \
                        elbow_vertical_angle <= self.thresholds['WRIST_ELBOW_VERT']['TRANS'][1] and \
                       self.state_tracker['state_seq'].count('s1')==1 :
                        self.state_tracker['CURL_SLOWLY'] = True  
                    ...
                    if (elbow_shoulder_hip > self.thresholds['ELBOW_SHOULDER_HIP_THRESH']) :
                ...
                if 's3' in self.state_tracker['state_seq'] or (current_state == 's2'  and not 's3' in self.state_tracker['state_seq']):
                    self.state_tracker['CURL_SLOWLY'] = False
                ...
                frame = self._show_feedback(frame, self.state_tracker['COUNT_FRAMES'], self.FEEDBACK_ID_MAP,self.state_tracker['HOLD_WEIGHT_AND_CURL'], self.state_tracker['CURL_SLOWLY'])
                ...
                cv2.putText(frame, str(int(elbow_shoulder_hip)), (hip_text_coord_x, hip_coord[1]), self.font, 1, self.COLORS['light_green'], 2, lineType=self.linetype)
            ...
            self.state_tracker['DONT_SWAY_YOUR_ELBOW_FORWARD'] = False
            # self.state_tracker['CORRECT_YOUR_HIPS'] = False
            self.state_tracker['DISPLAY_TEXT'] = np.full((1,), False)
            self.state_tracker['COUNT_FRAMES'] = np.zeros((1,), dtype=np.int64)
            self.state_tracker['start_inactive_time_front'] = time.perf_counter()
